...
class CheckerManager:
    ...
    async def _check_number(self, user_id, phone, checker_bot='@TelCheckers_bot'):
        try:
            client = self.get_client(user_id)
            if not client.is_connected():
            ...
            async with client.conversation(checker_bot, timeout=20) as conv:
    ...
    def check_number(self, user_id, phone, checker_bot='@TelCheckers_bot'):
        # FIX: Added 60s hard timeout — prevents background thread from hanging forever
        future = asyncio.run_coroutine_threadsafe(self._check_number(user_id, phone, checker_bot), self.loop)
        try:
...
def get_number_without_cc(phone_str, country_name):
    phone_clean = str(phone_str).replace('+', '').strip()
    if not country_name: return phone_clean
    cc_map = {
        'russia': '7', 'kazakhstan': '7', 'usa': '1', 'canada': '1',
        'england': '44', 'uk': '44', 'vietnam': '84', 'indonesia': '62',
        'india': '91', 'philippines': '63', 'myanmar': '95', 'macau': '853',
        'ivorycoast': '225', 'ukraine': '380', 'hong kong': '852', 'hongkong': '852',
        'cambodia': '855', 'thailand': '66', 'malaysia': '60', 'pakistan': '92',
        'bangladesh': '880', 'china': '86', 'brazil': '55', 'mexico': '52',
        'germany': '49', 'france': '33', 'spain': '34', 'italy': '39',
        'netherlands': '31', 'poland': '48', 'turkey': '90', 'egypt': '20',
        'south africa': '27', 'nigeria': '234', 'haiti': '509'
    }
    cc = cc_map.get(country_name.lower())
    if cc and phone_clean.startswith(cc):
        return phone_clean[len(cc):]
    return phone_clean
...
def show_main_menu(message):
    ...
    markup.add(
        KeyboardButton('📱 Buy Number'), KeyboardButton('💰 My Balance'),
        KeyboardButton('⚙️ Setup Checker'), KeyboardButton('🗑️ Remove Checker'),
        KeyboardButton('👥 Accounts'), KeyboardButton('🤖 Select Checker Bot'),
        KeyboardButton('📋 Copy Format'), KeyboardButton('🚪 Logout')
    )
...
def show_accounts_stats(message):
    ...
    yesterday_count = otp_dates.get(yesterday.isoformat(), 0)
    total_count = stats.get("total_otps", 0)
    
    text = (f"📈 *Accounts / OTP Stats*\n\n"
            f"📅 *Today's OTPs:* `{today_count}`\n"
            f"⏮️ *Yesterday's OTPs:* `{yesterday_count}`\n"
            f"📊 *Total OTPs:* `{total_count}`")
    bot.reply_to(message, text, parse_mode="Markdown")

@bot.message_handler(func=lambda message: message.text and 'Copy Format' in message.text)
@access_required
def select_copy_format(message):
    markup = InlineKeyboardMarkup(row_width=1)
    markup.add(
        InlineKeyboardButton("✅ With Country Code", callback_data="copyfmt_with"),
        InlineKeyboardButton("❌ Without Country Code", callback_data="copyfmt_without")
    )
    bot.reply_to(message, "📝 *Select how phone numbers should be copied:*", reply_markup=markup, parse_mode="Markdown")

@bot.callback_query_handler(func=lambda call: call.data.startswith('copyfmt_'))
def handle_copy_fmt(call):
    user_id = call.from_user.id
    fmt = call.data.split('_')[1]
    users_db[user_id]['copy_format'] = fmt
    save_db(db_data)
    bot.answer_callback_query(call.id, f"✅ Copy format set to: {'With' if fmt == 'with' else 'Without'} Country Code")
    bot.edit_message_text(f"✅ Copy format set to: *{'With' if fmt == 'with' else 'Without'} Country Code*", call.message.chat.id, call.message.message_id, parse_mode="Markdown")
...
@bot.message_handler(func=lambda message: message.text and 'Logout' in message.text)
@access_required
...
@bot.message_handler(func=lambda message: message.text == '🤖 Select Checker Bot')
@access_required
def select_checker_bot(message):
    markup = InlineKeyboardMarkup(row_width=1)
    markup.add(
        InlineKeyboardButton("✅ @TelCheckers_bot", callback_data="setbot_@TelCheckers_bot"),
        InlineKeyboardButton("✅ @Telvalidbot", callback_data="setbot_@Telvalidbot")
    )
    bot.reply_to(message, "🤖 *Select which bot to use for checking numbers:*", reply_markup=markup, parse_mode="Markdown")

@bot.callback_query_handler(func=lambda call: call.data.startswith('setbot_'))
def handle_set_bot(call):
    bot_username = call.data.split('_')[1]
    user_id = call.from_user.id
    init_user(user_id)
    users_db[user_id]['checker_bot'] = bot_username
    save_db(db_data)
    bot.answer_callback_query(call.id, f"✅ Checker bot set to {bot_username}")
    bot.edit_message_text(f"🤖 *Selected checker bot:* `{bot_username}`", call.message.chat.id, call.message.message_id, parse_mode="Markdown")
...
def start_add_prefix(call):
    ...
    users_db[call.from_user.id]['temp_data']['last_msg_id'] = call.message.message_id
    msg = bot.send_message(call.message.chat.id, "✍️ *Send me the prefix(es) separated by commas (e.g. 85257, 85246):*", parse_mode="Markdown")
    ...
    for attempt in range(1, 51):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # 2. CHECKER STATUS
                checker_bot_uname = users_db[user_id].get('checker_bot', '@TelCheckers_bot')
                print(f"Prefix OK. Checking status via {checker_bot_uname}...")
                db_data["stats"]["checked"] += 1
                status = checker.check_number(user_id, phone, checker_bot_uname)
                ...
                if keep_number:
                    ...
                    otp_val = "Waiting..."
                    phone_s = str(phone)
                    display_n = get_number_without_cc(phone_s, country)
                    cc_p = phone_s.replace('+', '').strip()[:-len(display_n)] if display_n else ""
                    fmt_pref = users_db[user_id].get('copy_format', 'without')
                    phone_display = f"`+{cc_p}{display_n}`" if fmt_pref == 'with' else f"+{cc_p}`{display_n}`"
                                ...
                                if sms_list:
                                    ...
                                    for sms in sms_list:
                                        ...
                                        if sms_text not in received_sms_texts:
                                            ...
                                            try:
                                                otp_markup.add(InlineKeyboardButton(text=f"{otp_val}", copy_text=telebot.types.CopyTextButton(text=str(otp_val))))
                                            except:
                                                otp_markup.add(InlineKeyboardButton(text=f"{otp_val}", callback_data="ignore"))
                                            bot.send_message(
                                                chat_id,
                                                f"+{str(phone).replace('+', '')}",
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if sms:
                msg_id = call.message.message_id
                ctx = users_db[user_id].get('purchase_context', {}).get(str(msg_id), {})
                service = ctx.get('service') or users_db[user_id]['temp_data'].get('service', 'Service')
                country = ctx.get('country') or users_db[user_id]['temp_data'].get('country', '')
                
                phone_str = str(data.get("phone", "Unknown")).replace("+", "")
                ...
                for s in sms:
                    ...
                    otp_markup = InlineKeyboardMarkup()
                    try:
                        otp_markup.add(InlineKeyboardButton(text=f"{otp_code}", copy_text=telebot.types.CopyTextButton(text=str(otp_code))))
                    except:
                        otp_markup.add(InlineKeyboardButton(text=f"{otp_code}", callback_data="ignore"))
...
def handle_prefix(message):
    ...
    new_prefixes = [p.strip() for p in input_text.split(',') if p.strip()]
    users_db[user_id]['state'] = 'MAIN_MENU'
    country = users_db[user_id]['temp_data'].get('country')
    if country and new_prefixes:
